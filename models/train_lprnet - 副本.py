...
# 注意：xavier方法需要单独定义，这里只是作为示例使用，实际使用时需要确保xavier方法可用。
   ...
   # 2.训练lprnet时最好使用adam，sgd会在ctc_loss0.2-0.3之间震荡
    ...
    for iteration in range(start_iter, max_iter):
        ...
        # load train data
        images, labels, lengths = next(batch_iterator) # 获取一个batch数据
        # labels = np.array([el.numpy() for el in labels]).T 转成numpy数组
        # print(labels)
        # get ctc parameters
